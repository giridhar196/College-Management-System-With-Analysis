...
$sqlc = "SELECT * FROM `blog` WHERE permission ='1' ORDER BY id DESC";
...
$sqlco = "SELECT COUNT(*) AS cou FROM `blog` WHERE permission ='1'";
...
if(isset($_GET['category']))  {
    $cat=$_GET['category'];
    $sqlc = "SELECT * FROM `blog` WHERE category = '$cat' AND permission ='1' ORDER BY id DESC";
    $run_sqla=mysqli_query($conn,$sqlc);  
    $l=0;
    echo $cat;
    if (mysqli_num_rows($run_sqla) > 0) {
    ...
    $sqlco = "SELECT COUNT(*) AS cou FROM `blog` WHERE category = '$cat' AND permission ='1'";
        $run_sqlac=mysqli_query($conn,$sqlco);  
