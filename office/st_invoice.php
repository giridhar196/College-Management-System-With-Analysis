...
if(isset($_POST["submit"])){
    ...
    $sql_stud = "SELECT * FROM `st_db` WHERE regdno='$adm_no'";
    ...
    if (mysqli_num_rows($run_sql_stud) > 0) {
        while($row = mysqli_fetch_assoc($run_sql_stud)) {
             ...
             $sql_count = "SELECT COUNT(id) as cc FROM `transaction` WHERE semister='$sm1' OR semister='$sm2' AND regdno='148x1a0532'";
    ...
    for ($jm=0;$jm<5;$jm++){
        $sql_invoice = "SELECT * FROM `transaction` WHERE categoryfor='${"type$jm"}' AND semister='1-1' OR semister='1-2' AND regdno='$adm_no'";
